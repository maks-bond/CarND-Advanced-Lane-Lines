...
[image14]: ./writeup_images/image13.jpg "Failed frame"
[image15]: ./writeup_images/image11.jpg "Y channel"
[image16]: ./writeup_images/image12.jpg "B channel"
...
To detect yellow line we use B channel of LAB color space. We select all pixels which are above 97th percentile of B channel values. Such a 97th percentile threshold is calculated in the lower half of the image to eliminate 'sky' area and increase influence of the 'road area' on percentile calculation.
...
The following source and destination points have been used:
...
5. Run sanity check on newly calculated polynomial coefficients to discard potentially incorrect polynomials. We only check that lane curvature of one lane is less than 3X curvature of another lane.
...
8. Calculate car center offset and draw it with lane curvatures on the image as text.
...
Here's a [link to project video](https://youtu.be/EUePcyq9vpQ)

## Challenge video
Here's a [link to challenge video](https://youtu.be/RAOqpin6k18)
...
This problem is probably the reason for majority of failures in the harder challenge video where the road is very curvy.
